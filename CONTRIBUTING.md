...
- Document new code following `AGENTS.md` § 2 (comments) and § 3 (JSDoc). Before committing, `npm run lint:comments` reports comment-style warnings; `npm run comments:rewrite --path=<folder>` (Claude Code, uses your own seat) rewrites them. Claude Code users may prefer the `Concise` output style (`/config` → Output style).
