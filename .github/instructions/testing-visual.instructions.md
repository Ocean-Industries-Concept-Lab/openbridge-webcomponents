...
`/update-snapshots` workflow could never import the Vitest setup file inside, so
confirm it still works before trusting a baseline it produces (see Open).
...
- The Docker route (`test-storybook:docker`) runs the image whose Vitest setup import failed in every `update-snapshots.yml` run; the workflow is retired, the image is unverified (#1179).
