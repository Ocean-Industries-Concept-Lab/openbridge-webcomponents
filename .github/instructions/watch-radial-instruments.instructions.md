...
| `clipTop` / `clipBottom` / `clipLeft` / `clipRight` | viewBox window in the **un-zoomed** path. Ignored when `zoomToFitArc` is true.                                                                                                          |
| `zoomToFitArc`                                      | Swaps to the `computeZoomToFitArcFrame()` path; every band radius gets the additive `_rOff` (see the `_bandRadius` INVARIANT in `watch.ts`).                                           |
| `arcFrame`                                          | Externally pre-computed zoom frame; only used when `zoomToFitArc` is on. If you pass it, you own keeping it in sync with `areas` / `watchCircleType` — `obc-watch` will not recompute. |
