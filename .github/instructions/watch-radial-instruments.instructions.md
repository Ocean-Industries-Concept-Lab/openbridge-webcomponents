...
**A partial arc also needs side crops.** The box is as wide as the whole
circle, so a 120° arc reaching `x = ±155.7` would otherwise sit ~60 units from
the side edges against ~20 above. compass-sector crops 8.1% per side, leaving
the 32 units the design frame does, and its zoom targets that same margin —
which is why a zoomed arc keeps the framing of the un-zoomed one instead of
being sized against a square.
