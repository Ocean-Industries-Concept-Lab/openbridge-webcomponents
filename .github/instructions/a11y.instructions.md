...
Keyboard behaviour is not a design decision to make from scratch. Before writing a key handler, look the widget up in the [WAI-ARIA Authoring Practices Guide (APG) patterns](https://www.w3.org/WAI/ARIA/apg/patterns/) — it is the industry-standard reference for how each widget type is expected to behave, and it is what screen-reader users and keyboard users have already learned from every other application. A component that invents its own bindings is harder to use, not more discoverable.

Work down this ladder and stop at the first step that applies:

1. **An APG pattern matches the widget.** Implement that pattern: its roles, its states, and its keyboard interaction table. Deliberately omitting part of a pattern is allowed when the omission is small and justified, but it must be stated in the implementation's JSDoc — an undocumented gap reads as a bug.
2. **No exact match, but a close one exists.** Adapt the closest pattern rather than starting over, and keep its key bindings intact for the parts that do map. Document which pattern you adapted and where the component deviates, so a reviewer can tell an intentional deviation from an oversight.
3. **Nothing in the APG applies.** Design a pattern, and then write it down — in the component's JSDoc and in the PR body. A new pattern must still: reuse conventional keys with their conventional meanings (`Enter`/`Space` activate, `Escape` cancels or backs out, arrow keys move within a composite widget), avoid overriding keys the browser or assistive technology owns, expose its state through ARIA so it is announced rather than only visible, and never leave the user stuck in a keyboard trap.

Link the pattern you followed from the implementation so the next reader can check the code against the spec. `TreeRovingNavigator` in [`src/internal/tree-roving-navigator.ts`](../../packages/openbridge-webcomponents/src/internal/tree-roving-navigator.ts) is the reference example of step 1: it links the [Tree View pattern](https://www.w3.org/WAI/ARIA/apg/patterns/treeview/), implements roving tabindex plus Up/Down, Left/Right and Home/End, and states that type-ahead and `*` are intentionally out of scope.

Step 3 is rare, but it does come up where a maritime interaction has no desktop-widget equivalent. `obc-slide-button` ([issue #1070](https://github.com/Ocean-Industries-Concept-Lab/openbridge-webcomponents/issues/1070)) is the open example: slide-to-confirm exists to force a deliberate action, and no APG pattern covers it, so the keyboard equivalent (e.g. arm with `Space`, confirm with a second `Space`, `Escape` to disarm) has to be designed and documented rather than borrowed. Deviating from the APG because a domain constraint demands it is fine; deviating by accident is not.
...
1. **Pattern check** — the interaction matches the APG pattern named in the component's JSDoc, or the deviation from it is documented there (§ 1).
2. **Tab through** the component in a story — every interactive element receives focus in a sensible order.
3. **Activate with keyboard only** — `Enter`, `Space`, arrow keys, `Escape` behave per the table in § 2.
