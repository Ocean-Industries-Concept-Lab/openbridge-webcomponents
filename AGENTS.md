...
15. **Never hand-edit `src/palettes/variables.css`.** It is regenerated wholesale from the [obc-figma-plugin](https://github.com/Ocean-Industries-Concept-Lab/obc-figma-plugin)'s `cssvariables` codegen — any local edit will be overwritten the next time someone pastes new plugin output. Token additions/renames must go through Figma (or the plugin's `rename()` function) first. The same caution applies to `script/figmavariables.json` (the plugin's `variables` codegen output). `src/mixins/fonts.css` is also seeded by the plugin's `font-exports` codegen, but the file has a **hand-curated section at the top** (banner-marked) that the codegen does not produce — preserve it when regenerating and run `npm run lint:mixins` afterwards. See [IMPLEMENTATION_GUIDELINES.md § PostCSS](IMPLEMENTATION_GUIDELINES.md#-postcss).
