...
4. **Code Organization**:
   ...
   - Common component structure pattern:
     ...
     - Observers as `private themeObserver?: MutationObserver` and `private resizeObserver?: ResizeObserver`; the latter comes from `observeLabelThreshold()` in `charthelpers/label-threshold.ts` (rebuild on a label-threshold crossing, update in place otherwise). The chart supplies only its threshold test; never hand-roll that observer
...
2. **File Organization**:
   - Common file structure:
     ...
     - `label-threshold.ts`: the resize observer that tells a threshold crossing (rebuild) from any other resize (update)
...
6. **Formatting Helpers**:
   - `formatChartNumber()` formats a value or a percentage for labels, legends and tooltips. It is not the readout stack's `formatNumericValue()` in `readout-formatters.ts`, which has a different signature
