---
name: figma-refresh
description: Regenerating variables.css, fonts.css, figmavariables.json and the icons from Figma, with the tripwires and the commit shape
globs:
  ...
  - packages/openbridge-webcomponents/script/download-icons.ts
  - packages/openbridge-webcomponents/script/convert-icons.ts
  - packages/openbridge-webcomponents/script/check-icon-hex-leaks.ts
...
from two Figma files and replaced wholesale. [`generated-code.md`](generated-code.md)
...
([`working-method.md` § Stop](working-method.md)):
...
3. `npm run palette:strip`, prettier again, and commit that second. The
   ...
   - every `Component-size` mode outside the four documented classes
     ...
     40 px touch target, below the floor [`a11y.md`](a11y.md) § 7
...
7. Run the full snapshot suite once without `--update`. A palette export
   ...
   ([`testing-visual.md`](testing-visual.md): filter before `--update`,
...
[`generated-code.md` § VariableID anatomy](generated-code.md#variableid-anatomy--why-a-refresh-can-lose-every-icon-colour).
...
| `OBC_ALLOW_UNRESOLVED_VARS=1`          | —       | let a run with unresolved ids exit 0; only for a token that does not exist in the palette yet, with a follow-up filed |
...
- **Every icon leaks hex, a few dozen ids unresolved** — the icons file
  ...
  ([`generated-code.md`](generated-code.md#variableid-anatomy--why-a-refresh-can-lose-every-icon-colour)).
- **The token exists, the map lacks it** — re-run `variables map` on the
  icons file, replace the JSON, re-run the download.
- **The token is not in the palette yet** — `OBC_ALLOW_UNRESOLVED_VARS=1`,
  accept the hex fallback, file the token with the design team, regenerate
  when the palette ships (#1187 is one).

`npm run lint:icons` (`script/check-icon-hex-leaks.ts`) fails on any literal
hex `fill`/`stroke` in `src/icons/*.ts`. A legitimate exception, such as a
regulatory colour that must not follow the theme, goes on an explicit
allowlist there, never on a sliding budget.
