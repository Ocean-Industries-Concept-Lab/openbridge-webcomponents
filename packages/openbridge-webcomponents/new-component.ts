...
const name = await question("Component name (without obc prefix, and UpperCamelCase) ?", {
  validators: [
    {
...
const componentType = await select("Type of component", { choices: ["ui(input, label, tables)", "instrument (compas, azimuth)"] });
...
const parentDir = componentType[0].includes("ui") ? "components" : "navigation-instruments";
