...
/**
 * Calculate canvas padding and formatted labels for donut chart
 *
...
export interface FixedHeightChartConfig {
  /** Fixed height in pixels (determines chart circumference) */
  fixedHeight: number;
  /** Whether chart is in half mode (180° arc instead of 360°) */
  isHalfMode: boolean;
...
export interface FixedHeightChartDimensions {
  ...
  visibleWidth: number;
  /** Actual rendered canvas height (differs from fixedHeight in half mode) */
  actualHeight: number;
