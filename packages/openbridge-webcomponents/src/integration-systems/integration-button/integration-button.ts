...
export class ObcIntegrationButton extends LitElement {
  ...
  @property({type: Array, attribute: false}) readouts: IntegrationButtonReadout[] = [];
  @property({type: Boolean}) disabled = false;
  @property({type: Boolean}) selected = false;

  override render() {
    return html`
      <button class="wrapper ${this.selected ? 'selected' : ''}" ?disabled=${this.disabled}>
        <div class="main-container">
          ...
          ${this.hasTrailingIcon ? html`<div class="icon-container">
            ${this.hasTrailingIcon2 ? html`<slot name="trailing-icon2" class="icon trailing"></slot>` : nothing}
        ...
        <div class="info-container">
          ...
          ${this.readouts.map(readout => html`
