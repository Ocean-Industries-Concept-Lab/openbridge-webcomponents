...
export const WithSetpointAtValue: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story: `**At-setpoint detection.** With \`autoAtSetpoint\` the marker shrinks to 80 % when \`|value − setpoint| ≤ autoAtSetpointDeadband\`; with it off, \`atSetpoint\` sets the state directly. \`setpointAtZeroDeadband\` snaps a near-zero setpoint onto the zero line. The rules are pinned in \`svghelpers/setpoint.spec.ts\`.`,
      },
    },
  },
