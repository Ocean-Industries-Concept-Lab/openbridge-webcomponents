  ...
  render: () => {
    return html`<obc-alert-list style="height: 300px; display: block;" empty>
      ...
      <div slot="empty-description">
        <span>Go to the 'Alert list' for more details or to manage existing alerts.</span>
      </div>
    </obc-alert-list>`;
  },
}
...
export const WithControls: Story = {
  ...
  render: (args, context) => {
    ...
    <div style="display: flex; flex-direction: column; gap: 10px;">
      <obc-button @click=${() => {
        const list = (context.canvasElement.querySelector('[data-testid="alert-menu"]') as ObcAlertList)
        const newItem = document.createElement('obc-alert-menu-item') as ObcAlertMenuItem
