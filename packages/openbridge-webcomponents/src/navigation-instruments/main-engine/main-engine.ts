...
import { atSetpoint, convertThrustAdvices, thrusterColors, thrusterTopSingleSided, setpointSvg } from '../thruster/thruster';
...
export class ObcMainEngine extends LitElement {
  ...
  override render() {
    ...
    const thrustAtSetpoint = atSetpoint(this.thrust, this.thrustSetpoint, { atSetpoint: this.atThrustSetpoint, autoAtSetpoint: !this.disableAutoAtThrustSetpoint, autoSetpointDeadband: this.autoAtThrustSetpointDeadband, touching: this.thrustTouching });
    const cThrust = thrusterColors({
      atSetpoint: thrustAtSetpoint,
      touching: this.thrustTouching,
    }, this.state);
    const speedAtSetpoint = atSetpoint(this.speed, this.speedSetpoint, { atSetpoint: this.atSpeedSetpoint, autoAtSetpoint: !this.disableAutoAtSpeedSetpoint, autoSetpointDeadband: this.autoAtSpeedSetpointDeadband, touching: this.speedTouching });
    ...
    const thrustTop = svg`<g transform="translate(44, 0)">
      ${thrusterTopSingleSided(174, Math.max(this.thrust, 0), { box: cThrust.boxColor, container: "" }, {
      hideContainer: true,
    ...
    const thrusterBottom = svg`<g transform="rotate(180) scale(-1,1) translate(44)">
     ${thrusterTopSingleSided(174, Math.max(-this.thrust, 0), { box: cThrust.boxColor, container: "" }, {
      ...
      flipAdicePattern: false,
      narrow: false,
    }, bottomThrustAdvice)}</g>`;
    const thrustSetpoint = this.thrustSetpoint !== undefined ?
      svg`<g transform="translate(44, 0)">${setpointSvg(174, this.thrustSetpoint, thrustSetpointAtZero, { fill: cThrust.setPointColor, stroke: 'var(--border-silhouette-color)' }, {
...
`;
    const speedSetpoint = this.speedSetpoint !== undefined ?
      svg`<g transform="scale(-1 1) translate(44, 176)">${setpointSvg(350, this.speedSetpoint, speedSetpointAtZero, { fill: cSpeed.setPointColor, stroke: 'var(--border-silhouette-color)' }, {
        inCommand: this.state === InstrumentState.inCommand,
        singleSided: true,
        narrow: false,
      })}</g>` : nothing;
