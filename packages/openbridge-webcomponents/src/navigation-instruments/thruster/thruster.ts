...
export class ObcThruster extends LitElement {
  ...
  @property({ type: Boolean, attribute: 'at-setpoint' }) atSetpoint: boolean = false;
  ...
  override render() {
    return thruster(this.thrust, this.size, this.setpoint, this.state, {atSetpoint:  this.atSetpoint, tunnel: this.tunnel})
  }
}

export function thruster(thrust: number, size: Size, setpoint: number | undefined, state: InstrumentState, options: {atSetpoint: boolean, tunnel: boolean}) {
