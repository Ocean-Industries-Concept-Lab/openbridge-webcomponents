...
export class ObcWind extends LitElement {
  ...
  @property({type: Array, attribute: false}) windHistogramData: WindHistogramData[] = [];
  @property({type: String}) vesselImage: VesselImage = VesselImage.genericTop;
  @property({type: Number}) vesselHeadingDeg: number = 0;
  override render() {
    return html`
      <div class="wrapper">
        <obc-watch .watchCircleType=${WatchCircleType.double} 
  ...
  private renderWindHistogram() {
    ...
    const maxOccurrences = Math.max(...this.windHistogramData.map(d => d.occurrences));
    ...
    if (this.windHistogramData.length > 1) {
      ...
      const sorted = [...this.windHistogramData].sort((a, b) => a.direction - b.direction);
      for (let i = 0; i < sorted.length; i++) {
        const curr = sorted[i];
        const next = sorted[(i + 1) % sorted.length];
        const start = curr.direction;
        // Ensure wrap-around at 360
        const end = next.direction > start ? next.direction : next.direction + 360;
        for (let deg = start; deg < end; deg++) {
          const t = (deg - start) / (end - start);
          const occ = curr.occurrences + t * (next.occurrences - curr.occurrences);
    ...
    for (let deg = 0; deg < 360; deg++) {
      ...
      const radius = maxRadius - (occ / maxOccurrences) * (maxRadius - minRadius);
    ...
    return html`
      <svg width="100%" height="100%" viewBox="-200 -200 400 400">
        <mask id="mask">
          ...
          <circle cx="0" cy="0" r="109" vector-effect="non-scaling-stroke" stroke="white" stroke-width="1" fill="transparent"/>

        </mask>
        <circle cx="0" cy="0" r="109" vector-effect="non-scaling-stroke" stroke="var(--instrument-regular-tertiary-color)" stroke-width="1" fill="var(--instrument-regular-tertiary-color)" mask="url(#mask)" />
      </svg>
    `;
  }
