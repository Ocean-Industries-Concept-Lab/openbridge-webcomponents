...
export class ObcElevatedCardRadio extends LitElement {
  ...
  override render() {
    return html`
      <obc-elevated-card class="obc-elevated-card-radio" .overrideTag=${ObcElevatedCardTag.Div} .position=${this.position} .size=${ObcElevatedCardSize.SingleLine} ?graphicBorder=${this.graphicBorder} ?border=${this.border} @click=${this._handleCardClick}>
      <obc-radio .name=${this.name} inputId=${this.instanceId} .value=${this.value} ?checked=${this.checked} ?disabled=${this.disabled} ?required=${this.required} slot="leading-icon"></obc-radio>
      <label class="obc-elevated-card-radio__label" for=${this.instanceId} slot="label">${this.label}</label>
      </obc-elevated-card>
    `
  }
