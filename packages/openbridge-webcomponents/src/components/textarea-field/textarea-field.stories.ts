...
export const RecordingPaused: Story = {
  args: {
    ...
    recordingDuration: 5,
    audioLevels: [0.3, 0.5, 0.7, 0.4, 0.8],
  },
};
    ...
    class extends LitElement {
      ...
      private _pauseRecording(): void {
        ...
        if (el) {
          ...
          this._updateStatus('Recording paused.');
        }
      }
        ...
        return html`
          <div>
            ...
            <obc-textarea-field
              ...
              ?hideToolbar=${args.hideToolbar}
              ?hideVoiceRecording=${args.hideVoiceRecording}
              @voice-recording-start=${() => this._startRecording()}
              @voice-recording-delete=${() => this._deleteRecording()}
              @voice-recording-pause=${() => this._pauseRecording()}
              @voice-recording-resume=${() => this._resumeRecording()}
              @voice-transcribe=${() => this._transcribeRecording()}
              @voice-send=${() => this._sendVoiceMessage()}
        ...
        return html`
          <div>
            ...
            <obc-textarea-field
              ...
              ?hideVoiceRecording=${args.hideVoiceRecording}
              @voice-recording-start=${() => this._startRecording()}
              @voice-recording-cancel=${() => this._cancelRecording()}
              @voice-recording-pause=${() => this._pauseRecording()}
              @voice-recording-resume=${() => this._resumeRecording()}
              @voice-recording-confirm=${() => this._confirmRecording()}
            ></obc-textarea-field>
          </div>
        `;
      }
