...
export class ObcContextMenuInput extends LitElement {
  ...
  private handleNavigationItemClick(option: ContextMenuOption, event: Event) {
    ...
    this.dispatchEvent(
      new CustomEvent<ObcContextMenuInputItemClickEvent['detail']>('item-click', {
    ...
    if (this.isMultiSelect) {
      ...
      newSelectedValues = [...this.selectedValues.filter((v) => !groupValues.includes(v)), option.value];
    } else {
      newSelectedValues = [option.value];
    }

    if (!arraysEqual(this.selectedValues, newSelectedValues)) this.updateSelection(newSelectedValues);
  }

  private findParentGroup(value: string, opts: ContextMenuOption[] = this.options): ContextMenuOption | null {
    for (const o of opts) {
  ...
  private handleCheckboxChange(option: ContextMenuOption, e: Event) {
    const { status } = (e as CustomEvent<{ status: 'checked' | 'unchecked' }>).detail;
    const next = status === 'checked' ? [...this.selectedValues, option.value] : this.selectedValues.filter((v) => v !== option.value);
  ...
  private renderTitleBar() {
    ...
    return html`<div class="title-container"><div class="title-content"><div class="title-text">${this.title}</div></div><obc-icon-button variant="flat" @click=${this.handleCloseClick} aria-label="Close menu"><obi-close-google></obi-close-google></obc-icon-button></div>`;
  }
  ...
  private renderNavItem(o: ContextMenuOption) {
    ...
    return html`<div class="menu-item navigation-item-wrapper" style=${indent ? `padding-left:${indent}px` : ''}><obc-navigation-item .label=${o.label} .checked=${isSelected} .variant=${ObcNavigationMenuVariant.Full} @click=${(e: Event) => this.handleNavigationItemClick(o, e)} role="menuitem" aria-selected=${isSelected}></obc-navigation-item></div>`;
  }
  ...
  private renderRadioItems() {
    return this.options.map((o) => {
      ...
      return html`<div class="menu-item radio-item-wrapper" style=${indent ? `padding-left:${indent}px` : ''}><obc-radio .label=${o.label} .name=${this.effectiveRadioGroupName} .value=${o.value} .inputId=${id} .checked=${isSelected} @change=${(e: Event) => this.handleRadioChange(o, e)}></obc-radio></div>`;
    });
  }
  ...
  private renderCheckboxItems() {
    return this.options.map((o) => {
      const isSelected = this.isOptionSelected(o.value);
      const isNested = this.type === 'nested-checkboxes' && o.level && o.level > 1;
      const indent = isNested ? (o.level! - 1) * 16 : 0;
      return html`<div class="menu-item checkbox-item-wrapper" style=${indent ? `padding-left:${indent}px` : ''}><obc-checkbox .label=${o.label} .status=${isSelected ? 'checked' : 'unchecked'} @change=${(e: Event) => this.handleCheckboxChange(o, e)}></obc-checkbox></div>`;
    });
  }
  ...
  private renderFlyoutChildren(children: ContextMenuOption[]) {
    return children.map((c) => {
      ...
      return html`<obc-navigation-item .label=${c.label} .checked=${isSelected} .variant=${ObcNavigationMenuVariant.Full} @click=${(e: Event) => this.handleNavigationItemClick(c, e)} role="menuitem" aria-selected=${isSelected}></obc-navigation-item>`;
    });
  }
  ...
  private renderFlyoutItems() {
    return this.options.map((o) => {
      ...
      if (o.children?.length) {
        return html`<obc-navigation-item-group .label=${o.label} .checked=${isSelected} .variant=${ObcNavigationMenuVariant.Full} .hug=${true} @open=${() => {
              this.shadowRoot?.querySelectorAll('obc-navigation-item-group').forEach((g) => {
                if ((g as any).label !== o.label) (g as any).close();
              });
  ...
  private renderMultiColumnItems() {
    ...
    if (this.type === 'multi-with-subtitles') {
      if (!this.columnGroups.length) {
        ...
        return html`<div class="multi-content"><div class="multi-columns"><div class="columns-container">${cols.map((col, idx) => html`<div class="column-with-header ${idx ? 'column-divider' : ''}"><div class="column-header"><div class="subtitle-container"><div class="subtitle-text">Group ${idx + 1}</div></div></div><div class="column-content">${renderColumn(col)}</div></div>`)}</div></div></div>`;
      }
      ...
      this.columnGroups.forEach((g, gIdx) => {
        const calculatedCols = Math.ceil(g.options.length / this.itemsPerColumn);
        const actualCols = Math.max(g.columns, calculatedCols);
        const distributed = this.chunkArray(g.options, this.itemsPerColumn, actualCols);
      ...
      return html`<div class="multi-content"><div class="multi-columns"><div class="columns-container">${allCols.map((c) => html`<div class="column-with-header ${!c.isFirstGroup && c.isFirstInGroup ? 'column-divider' : ''}">${c.isFirstInGroup ? html`<div class="column-header"><div class="subtitle-container"><div class="subtitle-text">${c.groupTitle}</div></div></div>` : html`<div class="column-header-spacer"></div>`}<div class="column-content">${renderColumn(c.options)}</div></div>`)}</div></div></div>`;
    }

    // Simple multi (no subtitles)
    const columns = this.chunkArray(this.options, this.itemsPerColumn);
    return html`<div class="multi-content"><div class="multi-columns">${columns.map((col, idx) => html`<div class="column ${idx ? 'column-divider' : ''}">${renderColumn(col)}</div>`)}</div></div>`;
  }
  ...
  override render() {
    return html`<div class=${classMap({ 'context-menu': true, [`type-${this.type}`]: true, 'has-title': this.hasTitleBar })} role="menu" aria-label=${this.hasTitleBar ? this.title : 'Context menu'}>${this.renderTitleBar()}<div class="menu-content">${this.renderMenuContent()}</div></div>`;
  }
