...
export class ObcRadio extends LitElement {
  ...
  override render() {
    ...
    return html`
      <label for=${ifDefined(this.value)} class="has-label obc-radio-button"
        @click=${this.onClick}
      >
      ...
      return html`
      <input class="obc-radio-button" type="radio" .name=${ifDefined(this.name)} .value=${ifDefined(this.value)}
        ?checked=${this.checked} ?disabled=${this.disabled} ?required=${this.required}> 
