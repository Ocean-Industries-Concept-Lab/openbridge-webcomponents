...
export class ObcValveAnalogThreeWayIcon extends LitElement {
  ...
  override render() {
    ...
    const valueRotation = 0.5 * this.value - 0.5 * this.value2 + 50;

    const handleRotation =
      -(1 - valueRotation / 100) * (this.horisontal ? 180 : 90);
    const handle = svg`
    ...
    const x =
      xmin +
      ((xmax - xmin) * (this.horisontal ? this.value2 : this.value)) / 100;
    ...
    const y =
      ymin +
      ((ymax - ymin) * (this.horisontal ? this.value : this.value2)) / 100;
