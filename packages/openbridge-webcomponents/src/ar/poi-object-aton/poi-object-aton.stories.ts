...
export const AlertStates: Story = {
  render: () => html`
    ${['caution', 'warning', 'alarm'].map(
      (state) => html`
        <div style=${sectionStyle}>
          <h3 style=${headerStyle}>${state} - All Types</h3>
          <div style=${rowStyle}>
            ${types.map(
              (type) => html`
                <div style=${itemStyle}>
                  <obc-poi-object-aton type=${type} .state=${state}>
                    <obi-beacon-general-east></obi-beacon-general-east>
                  </obc-poi-object-aton>
                  <div style=${labelStyle}>${type}</div>
                </div>
    ...
    <div style=${sectionStyle}>
      <h3 style=${headerStyle}>Alert States with Color Styles</h3>
      <div style=${rowStyle}>
        ${['caution', 'warning', 'alarm'].map(
          (state) => html`
            ${styles.map(
              (style) => html`
                <div style=${itemStyle}>
                  ...
                  <div style=${labelStyle}>${state}/${style}</div>
                </div>
              `
            )}
          `
