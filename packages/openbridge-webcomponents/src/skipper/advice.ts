...
// mainColor = advice.type == AdviceType.caution ? 'var(--on-caution-active-color)' : advice.type == AdviceType.warning ? 'var(--on-warning-active-color)' : advice.type == AdviceType.alarm ? 'var(--on-alarm-active-color)' : '';
// fillColor = advice.type == AdviceType.caution ? 'var(--on-caution-active-color)' : advice.type == AdviceType.warning ? 'var(--on-warning-active-color)' : advice.type == AdviceType.alarm ? 'var(--on-alarm-active-color)' : '';

export function renderAdvice(advice: AngleAdviceRaw): SVGTemplateResult {
  if ((Math.abs(advice.maxAngle - advice.minAngle)) < 4.5) {
    return svg``;
  }
  if (advice.type === AdviceType.caution || advice.type === AdviceType.warning || advice.type === AdviceType.alarm) {
    let mainColor;
    ...
    if (advice.state === AdviceState.hinted) {
      ...
      mainColor = advice.type == AdviceType.caution ? 'var(--on-caution-active-color)' : advice.type == AdviceType.warning ? 'var(--on-warning-active-color)' : advice.type == AdviceType.alarm ? 'var(--on-alarm-active-color)' : '';
      fillColor = advice.type == AdviceType.caution ? 'var(--alert-caution-color)' : advice.type == AdviceType.warning ? 'var(--alert-warning-color)' : advice.type == AdviceType.alarm ? 'var(--alert-alarm-color)' : '';
    }
    ...
    for (let i = 0; i < 360; i += 4) {
          radialPattern.push(
              svg`
                  <g transform="rotate(${i}) translate(-512 -512) ">
    ...
    return svg`
            ...
            <g mask="url(#${maskId})">
                ${fillColor ? svg`<rect x="-512" y="-512" width="1024" height="1024" fill="${fillColor}"/>` : nothing}
                ${advice.type == AdviceType.caution ? radialPattern : nothing}
            </g>
    ...
    if (advice.state === AdviceState.hinted) {
      ...
      mainColor = 'var(--on-caution-active-color)';
      // fillColor = 'var(--alert-caution-color)';  
      // mainColor = advice.type == AdviceType.warning ? 'var(--on-warning-active-color)' : advice.type == AdviceType.alarm ? 'var(--on-alarm-active-color)' : '';
      tickmarkStyle = TickmarkStyle.hinted;
    }
    return svg`
            ${adviceMask(advice.minAngle, advice.maxAngle, advice.state === AdviceState.regular ? mainColor : 'none', mainColor)}
            ${tickmark(advice.minAngle, {
              size: TickmarkType.primary,
              style: tickmarkStyle,
              scale: 1,
