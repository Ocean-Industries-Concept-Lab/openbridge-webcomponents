 ...
 * ```html
 * <obc-pie-chart data-sunburst></obc-pie-chart>
 * <script>
 *   document.querySelector('obc-pie-chart').data = [
 ...
 * ```html
 * <obc-pie-chart data-show-outer-labels data-outer-label-unit="kW" data-outer-label-decimal-places="2"></obc-pie-chart>
 * <script>
...
export class ObcPieChart extends LitElement {
  ...
  @property({type: Boolean, reflect: true, attribute: 'data-sunburst'})
  sunburst = false;
  ...
  override firstUpdated() {
    ...
    // Add hover and click listeners for sunburst interaction
    if (this.sunburst) {
      this.setupSunburstInteraction();
    }
  }
  ...
  private prepareChartData() {
    ...
    // Sunburst mode: create outer ring dataset
  ...
  private getChartOptions(): ChartOptions<'pie'> {
    ...
    // Use centralized layout calculation helper (pie charts don't have half mode)
  ...
  private createSunburstPlugin(
    ...
    return {
      ...
      afterDatasetsDraw: (chart) => {
        ...
        // Only draw if a segment is active (hovered or clicked)
  ...
  private updateChart() {
    ...
    // Simpler approach: just recreate the chart
    this.chart.destroy();
    this.createChart();
  }
