...
Upgrading from 1.x? The [Migration Guide](?path=/docs/introduction-migration-guide--docs) lists what changed in 2.0.
...
The project will **always remain open source**, but with the 1.0.0 release it has transitioned to a new licensing system. Each version and each commit in the repository follows a "delayed" permissive license cycle, set out in [LICENSE.txt](https://github.com/Ocean-Industries-Concept-Lab/openbridge-webcomponents/blob/develop/LICENSE.txt):
...
3. **JIP Members:** Members of the [Joint Industry Project](?path=/docs/introduction-join-the-project--docs) can obtain a **commercial license** that permits use before the Change Date without the AGPL obligations. Membership is granted by the project: you register through the [support form](https://docs.google.com/forms/d/e/1FAIpQLSc6JVRyyIease2PkUArirNkG0DeTB__YnWEovjVwISlaYdktA/viewform) and pay the membership fee; contributing code does not make you a member. A commercial license does not include redistribution under Apache 2.0 before the Change Date.
