...
// `npm run screenshots` exports images instead of comparing them, so it loads
// the stories without storybook-addon-vis. Every other run — CI included — is
// unaffected and keeps the snapshot path below.
const viteEnv = (
...
// Exported screenshots need the same determinism, so this is not gated.
beforeEach(async () => {
  await Promise.all(
