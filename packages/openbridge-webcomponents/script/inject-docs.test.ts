...
describe('docsFromManifest', () => {
  it('excludes private and protected members', () => {
    const d = docsFromManifest({
      modules: [{path: 'src/b.ts', declarations: [{kind: 'class', name: 'ObcB', members: [
        {kind: 'field', name: 'visible', description: 'Visible field.'},
        {kind: 'field', name: 'secret', description: 'x', privacy: 'private'},
        {kind: 'field', name: 'guarded', description: 'y', privacy: 'protected'},
      ]}]}],
    });
...
describe('injectDts', () => {
  ...
  it('escapes a `*/` inside a description so the injected comment cannot close early', () => {
    const closeDocs = docsFromManifest({
      modules: [{path: 'src/c.ts', declarations: [{kind: 'class', name: 'ObcC', members: [
        {kind: 'field', name: 'note', description: 'See the end marker `*/` here.'},
      ]}]}],
    });
    const src = 'export declare class ObcC extends LitElement {\n    note: string;\n}\n';
    expect(injectDts(src, closeDocs)).toBe('export declare class ObcC extends LitElement {\n    /** See the end marker `*\\/` here. */\n    note: string;\n}\n');
  });

  it('does not inject into a nested type literal, e.g. a getter\'s inline return type', () => {
    const nestedDocs = docsFromManifest({
      modules: [{path: 'src/d.ts', declarations: [{kind: 'class', name: 'ObcD', members: [
