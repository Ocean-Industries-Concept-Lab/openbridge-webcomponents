...
function diffValue(path: string, a: unknown, b: unknown, out: Difference[]): void {
  ...
  if (a && b && typeof a === 'object' && typeof b === 'object') {
    ...
    for (const k of keys) {
      ...
      else if (vb === undefined) out.push({path: sub, kind: 'missing', before: va});
...
export function compareManifests(a: Json, b: Json): Difference[] {
  ...
  for (const [p, modA] of ma) {
    ...
    for (const declA of da) {
      ...
      for (const key of ['members', 'attributes', 'events', 'slots', 'description']) {
...
if (process.argv[1] && process.argv[1].endsWith('compare-manifests.ts')) {
  ...
  for (const d of diffs) console.log(`${d.kind.padEnd(8)} ${d.path}${d.kind === 'changed' ? `\n  - ${JSON.stringify(d.before)}\n  + ${JSON.stringify(d.after)}` : ''}`);
