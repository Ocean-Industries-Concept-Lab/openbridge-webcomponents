 ...
 *
 * Why this matters: the framework wrappers are generated by `lit labs gen`, whose
 * analyzer discovers slots and events **only** from `@slot` / `@fires` JSDoc tags
 * — never from the template or from `dispatchEvent(...)` calls. If a component
 * renders a `<slot name="leading-icon">` but forgets the `@slot leading-icon` tag,
 * the slot is invisible to every downstream consumer (the framework wrappers, IDE
 * autocomplete, Storybook autodocs, code playgrounds). An undocumented event is
 * simply absent from the React/Vue/Angular/Svelte wrappers, so consumers get no
 * generated `onX` binding for it.
 *
 * Note that `cem analyze` (which produces `custom-elements.json`) is more lenient:
 * it *does* infer events from `this.dispatchEvent(new CustomEvent('name'))` even
 * with no tag, so a missing `@fires` can still surface in the manifest while being
 * absent from the wrappers. The manifest is therefore not a reliable signal that a
 * component is correctly documented — this audit is.
 ...
 * not mistaken for the component's own public API. An explicit type argument
 * (`new CustomEvent<{target: Poi}>('name')`) is optional and may itself contain
 * one level of nested angle brackets.
 */
function findDispatchedEvents(code: string): Set<string> {
  const events = new Set<string>();
  const re =
    /this\.dispatchEvent\(\s*new\s+(?:Custom)?Event\s*(?:<(?:[^<>]|<[^<>]*>)*>)?\s*\(\s*['"]([^'"]+)['"]/g;
  let m: RegExpExecArray | null;
