...
ChartJS.register(annotationPlugin, gradientPlugin, Title, Tooltip, CategoryScale, LinearScale, PointElement, LineElement, Filler)
...
function getCssVariableValue(variable: string): string {
  ...
  const value = getComputedStyle(ctx)
...
function updateGraph() {
  ...
  ChartJS.defaults.font.size = parseInt(getCssVariableValue("--global-typography-label-font-size").split("px")[0]);
  // Use 'Open Sans' as the fallback font
  ChartJS.defaults.font.family = getCssVariableValue("--font-family-main") + ", 'Open Sans', sans-serif";
  ChartJS.defaults.font.weight = parseInt(getCssVariableValue("--global-typography-label-font-weight"));
        ...
        gradient: {
          borderColor: {
            ...
            colors: {
              ...
              [target + .0001]: positiveStroke, // Red for values between 50.0001 and 100
              '100': positiveStroke,
            },
          },
        },
